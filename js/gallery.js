...
for (const card of cards) {
  card.addEventListener("click", (event) => {
    ...
    document.body.classList.add('_lock');//запрещаем скролить страницу при открытом слайдере
  });
}
...
sliderClose.addEventListener("click", (event) => {
    document.body.classList.remove("_lock");//разрешаем скролить страницу после закрытия слайдера
  event.preventDefault();
